...
if os.environ.get(constants.SPOTDL_ENABLED,
    ...
    from .external import spotdl_helper
    logging.warning(
        "You have enabled SPOTDL integration, make sure to configure the correct download path and check that you have enough disk space for music downloading.")

if os.environ.get(constants.LIDARR_ENABLED,
                  constants.LIDARR_ENABLED_DEFAULT_VALUE) == "1":
    from .external import lidarr_helper
    logging.warning(
        "You have enabled LIDARR integration, if an artist won't be found inside the lidarr database, the download process will be skipped.")
...
def get_subsonic_search_results(text_to_search):
    ...
    for set_search in set_searches:
        ...
        if "searchResult2" in subsonic_search and len(
                subsonic_search["searchResult2"]) > 0 and "song" in subsonic_search["searchResult2"]:
                ...
                if found is False:
                    ...
                    if artist_spotify != '' and "name" in artist_spotify:
                        logging.info(
                            ...
                            track['name'])
                        if "name" in track:
                            track, artist_spotify, found, excluded, song_ids, track_helper = match_with_subsonic_track(
                                track, artist_spotify, found, excluded, song_ids, track_helper, playlist_id, playlist_name)
                    if not excluded:
                        if os.environ.get(
                                constants.SPOTDL_ENABLED, constants.SPOTDL_ENABLED_DEFAULT_VALUE) == "1" and found is False:
                            if "external_urls" in track and "spotify" in track["external_urls"]:
                                is_monitored = True
                                if os.environ.get(
                                        constants.LIDARR_ENABLED, constants.LIDARR_ENABLED_DEFAULT_VALUE) == "1":
                                    is_monitored = lidarr_helper.is_artist_monitored(
                                        artist_spotify["name"])
                                if is_monitored:
                                    logging.warning(
                                        'Track %s - %s not found in your music library, using SPOTDL downloader',
                                        artist_spotify["name"],
                                        track['name'])
                                    logging.warning(
                                        'This track will be available after navidrome rescans your music dir')
                                    ...
                                    logging.warning(
                                        'This track hasn'
                                        't been found in your Lidarr database, skipping download process')
                        elif found is False:
                            logging.warning(
                                'Track %s - %s not found in your music library',
                                artist_spotify["name"],
...
def match_with_subsonic_track(track, artist_spotify, found,
                              excluded, song_ids, track_helper, playlist_id, playlist_name):
    text_to_search = artist_spotify["name"] + " " + track['name']
    ...
    for song_id in subsonic_search_results:
        ...
        if song["id"] in song_ids:
            ...
            album_name = track["album"]["name"] if (
                "album" in track and "name" in track["album"] and track["album"]["name"]) is not None else ""
            logging.info(
            ...
            if has_isrc(track):
                ...
                if found_isrc is True:
                    ...
                    found = True
                    database.insert_song(
                        dbms, playlist_id, song, artist_spotify, track)
                    logging.info(
                        ...
                        playlist_name,
                        isrc)
                    checkPysonicConnection().createPlaylist(
                        playlistId=playlist_id, songIds=song_ids)
                    break
            if (utils.compare_string_to_exclusion(song["title"], utils.get_excluded_words_array())
                    or utils.compare_string_to_exclusion(song["album"], utils.get_excluded_words_array())):
                excluded = True
            elif (utils.compare_strings(artist_spotify["name"], song["artist"])
    ...
    if found is False and excluded is False and len(skipped_songs) > 0:
        ...
        for skipped_song in skipped_songs:
            ...
            if placeholder not in track_helper:
                ...
                found = True
                database.insert_song(
                    dbms, playlist_id, skipped_song, artist_spotify, track)
                logging.warning(
                ...
                checkPysonicConnection().createPlaylist(
                    playlistId=playlist_id, songIds=song_ids)
    return track, artist_spotify, found, excluded, song_ids, track_helper
                    ...
                    if "playlist" in playlist_search:
                        ...
                        try:
                            if "subsonic_artist_id" in missing and missing["subsonic_artist_id"] is not None:
                                artist_search = checkPysonicConnection().getArtist(
                            ...
                            if "subsonic_song_id" in missing and missing["subsonic_song_id"] is not None:
                                song_search = checkPysonicConnection().getSong(
                        ...
                        if found_error:
                            logging.warning(
                                'Found a song inside Spotisub playlist %s with an unmatched Subsonic entry. Deleting this playlist.',
                            ...
                            try:
                                checkPysonicConnection().deletePlaylist(key)
                            except DataNotFoundError:
                                pass
                            break
